...
dm_patterns = [x for x in list1] + [(x + y) for x in list1 for y in list2] + [(x + y + z) for x in list1 for y in list2 for z in list2] + [(x + y + z + k) for x in list1 for y in list2 for z in list2 for k in list2]
...
for dm_pattern in dm_patterns:
    ...
    if not os.path.exists(label_summary_path) or overwrite:
        ...
        for subj in labelsl_df_left['Subj']:
            ...
            try:
                labels_percent = (labelsl_vals_left + labelsl_vals_right) * 100 * (1/(num_subj_left*len_sl_left+num_subj_right*len_sl_right))
        ...
        for reg in sorted_top_reg:
            ...
            struct_stripped = index_to_struct_comb[reg].replace('ctx-rh','').replace('ctx-lh','').replace('right','').replace('left','').replace('-','')
