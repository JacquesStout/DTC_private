...
for bundle in bundles:
    ...
    for subj in this_bundle_subjs:
        ...
        if savedatagrid_figs:
            ...
            figs_bundle_subj_path = os.path.join(figs_datagridcomparison,
                                                 f'subj_{subj[2:6]}_side_{side}_bundle_{bundle_num}')
...
for i, subj in enumerate(this_bundle_subjs):
    for bundle_id in np.arange(total_num_bundles):
        ...
        distance_var_FAbundle_dic[f'{subj[2:6]}', f'bundle_{int(bundle_id)}'] = norm(right_side_vars - left_side_vars)[
            0]
        distance_mean_FAbundle_dic[f'{subj[2:6]}', f'bundle_{int(bundle_id)}'] = \
        norm(right_side_means - left_side_means)[0]
        # distance_var_FAbundle_array[i, int(bundle_id)] = norm(right_side_coefs - left_side_coefs)[0]
        if save_fig:
            for side in ['left', 'right']:
                figs_plotvar_path = os.path.join(grid_var_path,
                                                 f'grid_var_{contrast}_{subj[2:6]}_{side}_bundle_{int(bundle_id)}')
...
distances_var_FAbundle_df.iloc[:,
distances_var_FAbundle_df.columns.get_loc('MRI_Exam')] = distances_var_FAbundle_df.iloc[:,
                                                         distances_var_FAbundle_df.columns.get_loc('MRI_Exam')].astype(
    str)
