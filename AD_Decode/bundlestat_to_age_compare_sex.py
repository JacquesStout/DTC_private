...
for sub_bundling_level in sub_bundling_levels:
    ...
    for group_column in group_columns:
        ...
        mkcdir(output_path)
        if combined:
            lm_path = os.path.join(output_path,'lm_age_wsex_combined_2')
        else:
            ...
            for stat_type in reg_stat_types:
                ...
                for col_type in col_tests:
                    ...
                    for i, bundle in enumerate(bundles):
                        ...
                        if genotype_included:
                            formula += f' * genotype'

                        model = sm.formula.ols(formula=formula, data=full_stat_db).fit()

                        if genotype_included:
                            interact_key = [key for key in model.pvalues.keys() if
                                            group_column in key and col_type in key and 'genotype' in key][0]
                        else:
                            interact_key = [key for key in model.pvalues.keys() if
                                            group_column in key and col_type in key and 'genotype' not in key][0]

                        group_key = [key for key in model.pvalues.keys() if group_column in key and bundle_stat not in key and col_type not in key][0]
                        ...
                        if formula_type == 'quadratic':
                            ...
                            #x_pred = np.linspace(full_stat_db[col_type].min(), full_stat_db[col_type].max(), 100)
                            group_values = full_stat_db[group_column]
                            colors = ['blue','red']
                            for group, color in zip(group_values,colors):
                                ...
                                plt.plot(x_pred, prediction_summary['mean'], color=color, label='Quadratic fit')
