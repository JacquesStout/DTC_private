...
if project == 'AD_Decode':
    ...
    anat_path = os.path.join(mainpath,
                             '/../mouse/VBM_21ADDecode03_IITmean_RPI_fullrun-work/dwi/SyN_0p5_3_0p5_fa/faMDT_NoNameYet_n37_i6/median_images/MDT_b0.nii.gz')
    space_param = '_MDT'
    ...
    anat_path = os.path.join(mainpath,
                             '../../mouse/VBM_19BrainChAMD01_IITmean_RPI_with_2yr-work/dwi/SyN_0p5_3_0p5_dwi/dwiMDT_Control_n72_i6/median_images/MDT_dwi.nii.gz')
...
for target_tuple in target_tuples:
    ...
    for group in groups:
        ...
        for ref in references:
            if empty_bundles[ref] > 0:
                print(f'Found {empty_bundles} empty bundles out of {np.size(
                    top_bundles)} for {ref} in group {group} for {region_connection}')
    ...
    for group in groups:
        ...
        if not os.path.exists(csv_summary) or overwrite:
            ...
            print('writing')
            groupcsvDF.to_csv(csv_summary, header=["Bundle Size Rank", "Bundle ID", "Streamlines ID", "Point ID",
                                                   "Length"] + references)
