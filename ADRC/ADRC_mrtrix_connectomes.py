...
if socket.gethostname().split('.')[0]=='santorini':
    ...
    root_proj = '/Volumes/Data/Badea/Lab/mouse/ADRC_jacques_pipeline/'
    ...
    root_proj = '/mnt/munin2/Badea/Lab/mouse/ADRC_jacques_pipeline/'
...
if len(sys.argv) > 1:
    ...
    tck_files = glob.glob(os.path.join(perm_input, '*.tck'))
    subjects = [os.path.basename(file)[:8] for file in tck_files]
...
SAMBA_path_results = os.path.join(root, 'mouse/VBM_23ADRC_IITmean_RPI-results/connectomics/')
