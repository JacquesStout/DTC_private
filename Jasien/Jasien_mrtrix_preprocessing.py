...
for subj in subjects:
    ...
    bvec_path_AP_unchecked = os.path.join(subj_out_folder, subj + '_bvecs_orig.txt')
    bval_path_AP_unchecked = os.path.join(subj_out_folder, subj + '_bvals_orig.txt')

    if verbose:
        print(f'Running subject {subj}')


    bxh_files = glob.glob(os.path.join(subj_folder, '*bxh'))
            ...
            with open(scan_path, 'r') as file:
                for line in file:
                    ...
                    if '<seriesdescription>' in line:
                        method_ser_name = line.split('>')[1].split('<')[0]
            #print(f'For {scan_path},\n the method is {method_name},\n the spat_resol is {method_rev_name}')
            if method_name == 'HCP DTI':
                DTI_forward_nii_path = scan_path.replace('.bxh','.nii.gz')
                DTI_forward_bxh = scan_path
            if method_name == 'HCP DTI reverse direction':
                DTI_reverse_nii_path = scan_path.replace('.bxh','.nii.gz')
                DTI_reverse_bxh = scan_path
    ...
    if not os.path.exists(bvec_path_PA) or not os.path.exists(bval_path_PA) or overwrite:
        ...
        with open(DTI_reverse_bxh) as file:
            for line in file:
                if "<value>" in line:
                    temp_loop = line
                    temp_loop_split = temp_loop.split()
        ...
        bvec_rvrs[1, 1:] = -bvec_rvrs[1, 1:]  # flip y sign
    ...
    if not os.path.exists(resampled_mif_path) or not os.path.exists(dwi_nii_gz) or not os.path.exists(mask_mif_path) \
            or not os.path.exists(coreg_bvecs) or overwrite:
        ...
        if not os.path.exists(mean_b0_PA_mif) or overwrite or resized:
            os.system(
                'mrconvert ' + PA_mif + ' -fslgrad ' + bvec_path_PA + ' ' + bval_path_PA + ' - | mrmath - mean ' + mean_b0_PA_mif + ' -axis 3 -force')
        ...
        if not os.path.exists(mean_b0_AP_mif) or overwrite:
            os.system(
                'dwiextract ' + output_denoise + ' - -bzero | mrmath - mean ' + mean_b0_AP_mif + ' -axis 3 -force')
        ...
        if not os.path.exists(masked_dwi_path) or not os.path.exists(mask_nii_path) or overwrite:
            median_mask_make(b0_dwi_nii_gz, masked_dwi_path, median_radius=median_radius,
                             binary_dilation=binary_dilation,
