...
import pandas as pd
from DTC.file_manager.computer_nav import load_nifti_remote, save_nifti_remote
...
for subj in subjects:
    ...
    if not os.path.exists(subj_anat_sambad) or not os.path.exists(subj_func_sambad):
        ...
        func_reorient_recentered_reslicedtargetaff = os.path.join(nii_temp_dir,
                                                                  f'{subj}_func_reorient_rigid_reslicedtargetaff{ext}')
        ...
        if not os.path.exists(func_reorient_recentered_reslicedtargetaff) or overwrite:
            ...
            if toreslice:
                ...
                _, targetreslice_affine = reslice(moved[..., 0], target_grid2world,
                                                  [0.1, 0.1, 0.1], [0.3, 0.3, 0.3])
                # _, movingreslice_affine = reslice(moved[..., 0], moving_grid2world,
                #                                  [0.1, 0.1, 0.1], [0.3, 0.3, 0.3])
                # save_nifti(func_reorient_recentered_reslicedaff, resliced, reslice_affine)
                # save_nifti(func_reorient_recentered_targetaff,resliced,target_grid2world)
                ...
                # save_nifti(func_reorient_recentered_reslicedmovingaff,resliced,movingreslice_affine)
            else:
            ...
            if not os.path.exists(subj_func_sambad) or overwrite:
                ...
                if np.size(glob.glob(os.path.join(split_subject_folder_transf, '*.nii.gz'))) < 1800:
                    for split_subj_file in split_subject_files:
                        ...
                        # cmd = f"antsApplyTransforms -v 1 -i {split_subj_file} -o {subj_func_sambad} -r {SAMBA_preprocess} -n MultiLabel -t [{rigid},0] [{trans},0]"
                        if not os.path.exists(output_transf):
                            cmd = f"antsApplyTransforms -v 1 -i {split_subj_file} -o {output_transf} -r {SAMBA_preprocess} -n MultiLabel -t [{rigid},0] [{trans},0]"
                            os.system(cmd)

                time_origin = 0
                time_spacing = float(
                    os.popen(f'fslval {func_reorient_recentered_reslicedtargetaff} pixdim4').read().split(' \n')[0])
        ...
        if transf_level == 'tofullwarp':
            ...
            if not os.path.exists(subj_func_sambad) or overwrite:
                # split_subject_folder = os.path.join(nii_temp_dir,f'{subj}_split')
                # mkcdir(split_subject_folder)
                # full_split_nii(func_reorient_recentered,split_subject_folder)
                cmd = f"antsApplyTransforms -v 1 -d 4 -i {func_reorient_recentered_reslicedtargetaff} -o {subj_func_sambad} -r {SAMBA_preprocess} -n MultiLabel -t {runno_to_MDT} [{affine_orig},0] [{rigid},0] [{trans},0]"
                os.system(cmd)
